...
var default_1$2 = (function (_super) {
    ...
    default_1.prototype.load = function (event) {
        ...
        var inputType;
        if (shared.MOUSE_DOWN === type && 0 === button) {
            this.target = target;
            this.isPressed = true;
            inputType = shared.INPUT_START;
        }
        else if (this.isPressed) {
            if (shared.MOUSE_MOVE === type) {
                inputType = shared.INPUT_MOVE;
            }
            else if (shared.MOUSE_UP === type) {
                points = [];
                inputType = shared.INPUT_END;
...
var default_1$3 = (function () {
    function default_1() {
        var Input = shared.SUPPORT_TOUCH ? default_1$1 : default_1$2;
    ...
    default_1.prototype.transform = function (event) {
        ...
        if (void 0 !== baseInputWithoutId) {
            ...
            var baseInput = tslib.__assign(tslib.__assign({}, baseInputWithoutId), { id: id });
            ...
            return tslib.__assign(tslib.__assign({}, pureInput), { prevInput: this.prevInput, startMultiInput: this.startMultiInput, startInput: this.startInput });
        }
    };
    return default_1;
}());
function getCenter(points) {
    ...
    if (0 < length) {
        ...
        var countPoint = points.reduce(function (countPoint, point) {
            countPoint.x += point[shared.CLIENT_X];
            countPoint.y += point[shared.CLIENT_Y];
            return countPoint;
        }, { x: 0, y: 0 });
...
function extendInput(inputBase) {
    ...
    var isStart = shared.INPUT_START === inputType;
    var isEnd = (shared.INPUT_END === inputType && 0 === pointLength) || shared.INPUT_CANCEL === inputType;
    var timestamp = performance.now();
    var _a = getCenter(points) || getCenter(changedPoints), x = _a.x, y = _a.y;
    var currentTarget = nativeEvent.currentTarget;
    return tslib.__assign(tslib.__assign({}, inputBase), { x: x, y: y,
        timestamp: timestamp,
...
function dispatchDomEvent (el, payload, eventInit) {
    var target = payload.target, currentTarget = payload.currentTarget, type = payload.type, data = tslib.__rest(payload, ["target", "currentTarget", "type"]);
    var event;
...
function canPreventDefault (event, options) {
    ...
    if (null !== event.target) {
        ...
        if (shared.isRegExp(preventDefaultExclude)) {
            if ('tagName' in event.target) {
                var tagName = event.target.tagName;
                isPreventDefault = !preventDefaultExclude.test(tagName);
            }
        }
        else if (shared.isFunction(preventDefaultExclude)) {
...
var TOUCH_EVENT_NAMES = [shared.TOUCH_START, shared.TOUCH_MOVE, shared.TOUCH_END, shared.TOUCH_CANCEL];
function bindElement (el, callback, options) {
    if (shared.SUPPORT_TOUCH) {
        ...
        el.addEventListener(shared.MOUSE_DOWN, callback, options);
        window.addEventListener(shared.MOUSE_MOVE, callback, options);
        window.addEventListener(shared.MOUSE_UP, callback, options);
        return function () {
            el.removeEventListener(shared.MOUSE_DOWN, callback);
            window.removeEventListener(shared.MOUSE_MOVE, callback);
            window.removeEventListener(shared.MOUSE_UP, callback);
        };
    }
}
...
function removeUse(instanceOrClass, recognizerName) {
    ...
    if (void 0 === recognizerName) {
        ...
        try {
            for (var _b = tslib.__values(instanceOrClass.recognizers.entries()), _c = _b.next(); !_c.done; _c = _b.next()) {
                var _d = tslib.__read(_c.value, 2), index = _d[0], recognizer = _d[1];
...
function emit2(at, payload) {
    ...
    if (!!at.options.domEvents
        ...
        dispatchDomEvent(target, tslib.__assign(tslib.__assign({}, payload), { type: AT_AFTER }), at.options.domEvents);
    }
}
...
var AnyTouch = (function (_super) {
    tslib.__extends(AnyTouch, _super);
    function AnyTouch(el, options) {
        ...
        _this.options = tslib.__assign(tslib.__assign({}, DEFAULT_OPTIONS), options);
    ...
    AnyTouch.prototype.catchEvent = function (event) {
        ...
        if (void 0 !== input) {
            ...
            if (false !== domEvents) {
                ...
                if (null !== target) {
                    dispatchDomEvent(target, tslib.__assign(tslib.__assign({}, input), { type: AT_TOUCH }), domEvents);
                    dispatchDomEvent(target, tslib.__assign(tslib.__assign({}, input), { type: AT_TOUCH_WITH_STATUS }), domEvents);
                }
            }
            ...
            var _loop_1 = function (recognizer) {
                ...
                recognizer.recognize(input, function (type, ev) {
                    var payload = tslib.__assign(tslib.__assign(tslib.__assign({}, input), ev), { type: type, baseType: recognizer.name });
            ...
            var this_1 = this;
            try {
                for (var _b = tslib.__values(this.recognizers), _c = _b.next(); !_c.done; _c = _b.next()) {
